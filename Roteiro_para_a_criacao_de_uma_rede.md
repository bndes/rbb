...
Após a instituição inicial começar a implantação da rede, as outras instituições vão entrar uma a uma. Os passos dessa seção serão executados a cada instituição que adira. 
...
- Criar um arquivo static-nodes.json para o validator com os validators das outras instituições (usando IPs externos) e com o boot da própria instituição (usando IP interno).
- Criar um arquivo static-nodes.json para o writer apenas com o boot da própria instituição (usando IP interno).

Os enodes que serão inseridos nos arquivos genesis.json e statis-nodes.json podem ser obtidos no arquivo anteriormente compartilhado em https://github.com/RBBNet/participantes/tree/main/${rede}/enodes.md. 
